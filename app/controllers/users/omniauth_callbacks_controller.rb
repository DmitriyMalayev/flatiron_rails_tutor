...
#above must match what's in routes.rb  #It allows us to affect only the things we want and everything else will be left intact.     
    def google_oauth2
        ...
        if user.present?
            sign_out_all_scopes  #Sign out all active users or scopes. This helper is useful for signing out all roles in one click. 
            flash[:success] = t 'devise.omniauth_callbacks.success', kind: 'Google'  #What is t ?? 
